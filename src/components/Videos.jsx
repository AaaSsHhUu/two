    ...
    <Stack direction={['column', 'row']} h={'100vh'}>
      <VStack w={'full'}>
        ...
        <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflowY={'auto'}>
          ...
          <Text>This is a Sample Video for demo.</Text>
        </VStack>
      </VStack>
      ...
      >
        {videosArr.map((item, index) => {
            const isPlaying = item === videoSrc
          return (<Button variant={isPlaying ? 'solid' : 'ghost'}  colorScheme="purple" onClick={()=> setVideoSrc(item)}>
            Video {index + 1}
          </Button>)
        })}
      </VStack>
